...
export class JWTControl {
  ...
  readonly secretOrPrivateKey: Secret = this.opts?.secret ?? this.opts?.privateKey ?? defaultPrivateKeyRS512;
  readonly secretOrPublicKey: string | Buffer = checkWarningPublicKeyVulnerable(this.opts?.secret ?? this.opts?.publicKey ?? defaultPublicKeyRS512);
  ...
  decode(token: string, options?: DecodeOptions) {
    return <any>decode(token, options);
  }

  verify(token: string, options?: VerifyOptions) {
    return <any>verify(token, this.secretOrPublicKey, {
      ...this.verifyOptions,
      ...options,
    });
  }
  ...
  async toJWK() {
    return {
      ...await fromKeyLike(crypto.createPublicKey({
